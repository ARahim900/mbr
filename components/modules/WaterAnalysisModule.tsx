...
const ModuleNavigation = ({ sections, activeSection, onSectionChange }: { sections: any[], activeSection: string, onSectionChange: (id: string) => void }) => {
  console.log('ModuleNavigation rendered:', { sections, activeSection });
  
  return (
    <div 
      className="bg-white dark:bg-gray-800 shadow-md rounded-lg p-2 mb-6 border border-neutral-border dark:border-gray-700 navigation-container navigation-debug"
      style={{ 
        position: 'relative', 
        zIndex: 50,
        minHeight: '60px',
    ...
    >
      <nav className="flex flex-wrap items-center justify-center gap-2" style={{ minHeight: '40px' }}>
        {sections.map((section, index) => {
          ...
          return (
            <button
              key={section.id}
              onClick={() => {
                console.log('Navigation clicked:', section.id);
...
const WaterAnalysisModule: React.FC = () => {
  ...
  console.log('WaterAnalysisModule state:', { activeWaterSubSection });
